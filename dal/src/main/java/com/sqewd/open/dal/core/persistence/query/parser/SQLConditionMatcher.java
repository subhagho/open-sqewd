...
import com.sqewd.open.dal.core.persistence.query.matcher.AndCondition;
import com.sqewd.open.dal.core.persistence.query.matcher.ColumnConstant;
import com.sqewd.open.dal.core.persistence.query.matcher.ExpressionCondition;
import com.sqewd.open.dal.core.persistence.query.matcher.OrCondition;
import com.sqewd.open.dal.core.persistence.query.matcher.QueryCondition;
import com.sqewd.open.dal.core.persistence.query.matcher.SQLConstant;
import com.sqewd.open.dal.core.persistence.query.matcher.ValueConstant;
...
public class SQLConditionMatcher extends ConditionMatcher {
	...
	private QueryCondition convert(final ZExp exp) throws Exception {
	...
	private QueryCondition convertExpression(final ZExp exp) throws Exception {
		if (exp instanceof ZExpression) {
			ZExpression ze = (ZExpression) exp;
			String op = ze.getOperator();
	...
	private QueryCondition convertAnd(final ZExp exp) throws Exception {
		if (exp instanceof ZExpression) {
			ZExpression ze = (ZExpression) exp;
			if (ze.getOperator().compareToIgnoreCase("AND") == 0) {
				QueryCondition left = convert(ze.getOperands().get(0));
				QueryCondition right = null;
				if (ze.nbOperands() == 2) {
	...
	private QueryCondition convertAnd(final Vector<ZExp> exps, final int index)
			throws Exception {
		ZExp exp = exps.elementAt(index);
		if (exp instanceof ZExpression) {
			ZExpression ze = (ZExpression) exp;
			QueryCondition left = convert(ze);
			QueryCondition right = null;
	...
	private QueryCondition convertOr(final ZExp exp) throws Exception {
		if (exp instanceof ZExpression) {
			ZExpression ze = (ZExpression) exp;
	...
	private QueryCondition convertOr(final Vector<ZExp> exps, final int index)
			throws Exception {
		ZExp exp = exps.elementAt(index);
		if (exp instanceof ZExpression) {
			ZExpression ze = (ZExpression) exp;
