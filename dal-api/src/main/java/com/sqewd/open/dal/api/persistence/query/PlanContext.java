...
public class PlanContext {
	...
	/**
	 * Check if the name is already used, if so, create a new alias.
	 * 
	 * @param name
	 *            - Entity name to check for.
	 * @return
	 */
	public synchronized String createNewAlias(final String name) {
		if (isNodeKeyUsed(name)) {
			StringBuffer alias = new StringBuffer(name);
			alias.append(_ALIAS_SUFFIX_).append(aliasindx);
			aliasindx++;
